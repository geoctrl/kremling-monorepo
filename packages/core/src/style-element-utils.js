...
export function newCssState(css, namespaceProp, defaultNamespace) {
  const isPostCss = Boolean(css && css.id)
  const namespace = (isPostCss ? css.namespace : namespaceProp) || defaultNamespace;
  ...
  if (!isPostCss) {
    ...
    if (css.indexOf("&") < 0 && css.trim().length > 0) {
      const firstRule = css.substring(0, css.indexOf("{")).trim();
      console.warn(
